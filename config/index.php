...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .folder-preview-pane {
      display: none; /* deprecated inline preview pane - replaced by modal preview */
      padding: 14px;
      position: sticky;
      top: 12px;
      min-height: 620px;
...
<body>
  ...
  <div id="view-folder-tab" class="tab-content">
    <div class="wrap folders-wrap">
      <div class="folders-workspace" id="foldersWorkspace">
        ...
        <aside class="folder-preview-pane" id="folderPreviewPane" style="display:none;">
  ...
  <div id="newsfeed-tab" class="tab-content newsfeed-main-only">
    <style>
      ...
      .library-thumb img.library-thumb-img { width: 100%; height: 100%; object-fit: cover; display: block; }
      .library-thumb .library-thumb-fallback { font-size: 28px; color: var(--brand); }

      /* Preview modal overlay */
      #library-preview-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; display: none; z-index: 2000; pointer-events: none; }
      #library-preview-overlay.open { display: block; pointer-events: auto; }
      #library-preview-modal { position: absolute; width: min(720px, 90vw); max-height: 90vh; background: #fff; border-radius: 10px; box-shadow: 0 12px 40px rgba(0,0,0,0.25); overflow: hidden; opacity: 0; transform: translateY(8px) scale(0.98); transition: opacity 200ms ease, transform 200ms ease; }
      #library-preview-overlay.open #library-preview-modal { opacity: 1; transform: translateY(0) scale(1); }
      #library-preview-modal .preview-body { display: flex; gap: 12px; padding: 12px; align-items: flex-start; }
      #library-preview-modal canvas, #library-preview-modal img.preview-img { max-width: 60vw; width: 100%; height: auto; display: block; }
      #library-preview-modal .preview-meta { padding: 12px; font-size: 0.95rem; color: #333; }
      #library-preview-modal .preview-close { position: absolute; top: 8px; right: 8px; background: transparent; border: none; font-size: 20px; cursor: pointer; color: #666; }
      .library-card-body {
        padding: 10px;
        display: flex;
        flex-direction: column;
...
<script>
  ...
  function previewFolderItem(item) {
    // Replace inline folder preview pane with modal preview for files.
    if (!item) return;
    if (item.kind === 'folder') {
      // For folders, we don't show an inline preview; user navigates into folder instead.
      return;
    }
    ...
    const uploaderEl = modal.querySelector('.preview-uploader');
    const dateEl = modal.querySelector('.preview-date');
    const sourceEl = modal.querySelector('.preview-source');
    const canvas = modal.querySelector('#pdf-preview-canvas');
    const img = modal.querySelector('#image-preview');

    titleEl.textContent = item.name || '';
    uploaderEl.textContent = item.owner || '';
    dateEl.textContent = formatDate(item.modifiedTime || item.createdTime) || '';
    sourceEl.textContent = 'Google Drive';

    if (canvas) { canvas.style.display = 'none'; const ctx = canvas.getContext && canvas.getContext('2d'); if (ctx) ctx.clearRect(0,0,canvas.width||0,canvas.height||0); }
    if (img) { img.style.display = 'none'; img.src = ''; }

    const fileUrl = item.url || getShareUrl(item) || '';
    const mimeType = (item.mimeType || '').toLowerCase();
    const previewUrl = item.id ? `https://drive.google.com/thumbnail?id=${item.id}&sz=w1200` : fileUrl;

    overlay.style.display = 'block';
    setTimeout(() => overlay.classList.add('open'), 20);

    if (mimeType.includes('pdf') || (fileUrl && fileUrl.toLowerCase().endsWith('.pdf'))) {
      if (window.pdfjsLib) {
        renderPDFToCanvas(fileUrl, modal.querySelector('#pdf-preview-canvas')).catch((err) => {
    ...
    } else if (mimeType.startsWith('image') || (/\.(png|jpg|jpeg|gif|bmp|svg|webp)$/i).test(previewUrl)) {
      if (img) { img.src = previewUrl; img.style.display = 'block'; }
    } else {
      if (img) { img.src = previewUrl; img.style.display = 'block'; }
    }
  ...
  function createLibraryCardHTML(entry) {
    ...
    const thumbHtml = entry.previewImage
      ? `<img class="library-thumb-img" src="${entry.previewImage}" alt="${escapeHtml(entry.title)}">`
      : `<i class="fas ${iconClass} library-thumb-fallback"></i>`;

    const fileUrl = (entry.asset && entry.asset.url) ? entry.asset.url : '';
    const fileType = (entry.asset && entry.asset.type) ? entry.asset.type : '';

    return `
      <div class="col-12 col-sm-6 col-xl-4 d-flex">
        <article class="library-card" data-post-id="${entry.postId}" data-file-url="${escapeHtml(fileUrl)}" data-file-type="${escapeHtml(fileType)}" data-preview-url="${escapeHtml(entry.previewImage || '')}" data-title="${escapeHtml(entry.title)}" data-uploader="${escapeHtml(entry.uploader)}" data-created="${escapeHtml(entry.createdLabel)}" data-source="${escapeHtml(entry.sourceLabel)}">
          ...
          <div class="library-thumb">${thumbHtml}</div>
          <div class="library-card-body">
            ...
            <div class="library-meta">
              ...
              <span><strong>Date:</strong> ${escapeHtml(entry.createdLabel)}</span>
  ...
  function renderLibraryGroups(records) {
    ...
    // Attach thumbnail image error handlers and show fallback icons when images fail
    Array.from(container.querySelectorAll('.library-thumb img.library-thumb-img')).forEach((img) => {
      img.addEventListener('error', function () {
        this.style.display = 'none';
        const fallback = this.parentElement && this.parentElement.querySelector('.library-thumb-fallback');
        if (fallback) fallback.style.display = 'block';
      });
    });
    // Attach preview handlers (hover / tap -> modal)
    try { attachPreviewHandlers(); } catch (e) { console.warn('attachPreviewHandlers error', e); }
    updateLibraryBatchBar();
  }
      ...
      if (result.isConfirmed) {
        try {
          ...
          try {
            ...
            console.debug && console.debug('delete_post invalid json', response.status, e);
            throw new Error('Invalid server response');
          }
          console.debug && console.debug('delete_post response', response.status, data);
...
<script>
  ...
  /* Library preview modal + PDF rendering helpers */
  let _previewCloseTimer = null;

  function attachPreviewHandlers() {
    console.debug && console.debug('attachPreviewHandlers scanning cards');
    // intentionally allow re-scanning cards after each render; per-card guards prevent duplicates
    const supportsHover = window.matchMedia ? window.matchMedia('(hover: hover)').matches : false;

    if (!window._libraryPreviewGlobalEventsAttached) {
      window._libraryPreviewGlobalEventsAttached = true;
      // global outside-click & escape to close
      document.addEventListener('click', function(e) {
        const overlay = document.getElementById('library-preview-overlay');
        const modal = document.getElementById('library-preview-modal');
        if (!overlay || overlay.style.display === 'none') return;
        if (modal && !modal.contains(e.target) && !e.target.closest('.library-card')) {
          closePreviewModal();
        }
      });
      document.addEventListener('keydown', function(e) { if (e.key === 'Escape') closePreviewModal(); });

      const modalEl = document.getElementById('library-preview-modal');
      if (modalEl) {
        modalEl.addEventListener('mouseenter', function() { if (_previewCloseTimer) { clearTimeout(_previewCloseTimer); _previewCloseTimer = null; } });
        modalEl.addEventListener('mouseleave', function() { startClosePreviewTimer(); });
      }
    }

    const cards = document.querySelectorAll('#posts-container .library-card[data-post-id]');
    cards.forEach((card) => {
      if (card.dataset.previewAttached === '1') return;
      card.dataset.previewAttached = '1';
      if (!supportsHover) {
        card.addEventListener('click', function(e) {
          if (e.target.closest('input[type="checkbox"], .library-card-actions, button, a')) return;
          openPreviewModalForCard(card);
        });
      } else {
        card.addEventListener('mouseenter', function() {
          if (card._hoverTimer) clearTimeout(card._hoverTimer);
          card._hoverTimer = setTimeout(() => openPreviewModalForCard(card), 2000);
        });
        card.addEventListener('mouseleave', function() {
          if (card._hoverTimer) { clearTimeout(card._hoverTimer); card._hoverTimer = null; }
          if (window._currentPreviewCardId === card.dataset.postId) startClosePreviewTimer();
        });
      }
    });
  ...
  function openPreviewModalForCard(card) {
    console.debug && console.debug('openPreviewModalForCard', card && card.dataset && card.dataset.postId);
    // clear any pending close timer so modal doesn't immediately close when overlay is shown
    if (_previewCloseTimer) { clearTimeout(_previewCloseTimer); _previewCloseTimer = null; }
    if (card && card._hoverTimer) { clearTimeout(card._hoverTimer); card._hoverTimer = null; }
    const overlay = document.getElementById('library-preview-overlay');
    const modal = document.getElementById('library-preview-modal');
    if (!overlay || !modal) return;
    const titleEl = modal.querySelector('.preview-title');
    ...
    uploaderEl.textContent = card.dataset.uploader || '';
    ...
    // position near card on desktop, center on touch devices
    const rect = card.getBoundingClientRect();
    const modalWidth = Math.min(720, Math.floor(window.innerWidth * 0.9));
    const supportHover = window.matchMedia ? window.matchMedia('(hover: hover)').matches : false;
    ...
    if (!supportHover) {
      left = Math.max(12, Math.floor((window.innerWidth - modalWidth) / 2));
      top = Math.max(12, Math.floor((window.innerHeight - 400) / 2));
    }
    ...
    overlay.style.display = 'block';
    setTimeout(() => overlay.classList.add('open'), 20);
    window._currentPreviewCardId = card.dataset.postId;

    if (fileType.includes('pdf') || fileUrl.toLowerCase().endsWith('.pdf')) {
      if (window.pdfjsLib) {
        renderPDFToCanvas(fileUrl, modal.querySelector('#pdf-preview-canvas')).catch((err) => {
          console.warn('PDF render failed', err);
          if (previewUrl && modal.querySelector('#image-preview')) { modal.querySelector('#image-preview').src = previewUrl; modal.querySelector('#image-preview').style.display = 'block'; }
        });
      } else {
        if (previewUrl && modal.querySelector('#image-preview')) { modal.querySelector('#image-preview').src = previewUrl; modal.querySelector('#image-preview').style.display = 'block'; }
      }
    } else if (previewUrl) {
      if (modal.querySelector('#image-preview')) { modal.querySelector('#image-preview').src = previewUrl; modal.querySelector('#image-preview').style.display = 'block'; }
    } else if (fileUrl) {
      if (modal.querySelector('#image-preview')) { modal.querySelector('#image-preview').src = fileUrl; modal.querySelector('#image-preview').style.display = 'block'; }
    }
  }

  function renderPDFToCanvas(url, canvas) {
    return new Promise(async (resolve, reject) => {
      if (!window.pdfjsLib) { reject('pdfjs unavailable'); return; }
      try {
        ...
        const containerWidth = Math.min(720, Math.floor(window.innerWidth * 0.9));
  ...
  function startClosePreviewTimer() {
    if (_previewCloseTimer) clearTimeout(_previewCloseTimer);
    _previewCloseTimer = setTimeout(() => closePreviewModal(), 1000);
  }
  ...
  function closePreviewModal() {
    ...
    setTimeout(() => {
      ...
      const canvas = modal.querySelector('#pdf-preview-canvas');
      const img = modal.querySelector('#image-preview');
      if (canvas && canvas.getContext) { const ctx = canvas.getContext('2d'); if (ctx) ctx.clearRect(0,0,canvas.width||0,canvas.height||0); canvas.style.display = 'none'; }
      if (img) { img.style.display = 'none'; img.src = ''; }
      window._currentPreviewCardId = null;
    }, 220);
  }
</script>

<!-- Library preview modal (opened on hover or tap) -->
<div id="library-preview-overlay" aria-hidden="true">
  <div id="library-preview-modal" role="dialog" aria-modal="true" aria-label="Preview">
    <button class="preview-close" aria-label="Close preview" onclick="closePreviewModal()">×</button>
    <div class="preview-body">
      <div class="preview-media" style="flex:1; min-width:200px;">
        <canvas id="pdf-preview-canvas" style="display:none; width:100%; height:auto;"></canvas>
        <img id="image-preview" class="preview-img" style="display:none; max-width:100%; height:auto;" alt="Preview image">
      </div>
      <div class="preview-meta" style="flex:0 0 260px;">
        <h4 class="preview-title"></h4>
        <div><strong>Uploaded by:</strong> <span class="preview-uploader"></span></div>
        <div><strong>Date:</strong> <span class="preview-date"></span></div>
        <div><strong>Source:</strong> <span class="preview-source"></span></div>
      </div>
    </div>
  </div>
</div>

<!-- PDF.js (used only for PDF previews) -->
<script src="https://unpkg.com/pdfjs-dist@2.16.105/build/pdf.min.js"></script>
<script>if(window.pdfjsLib){pdfjsLib.GlobalWorkerOptions.workerSrc='https://unpkg.com/pdfjs-dist@2.16.105/build/pdf.worker.min.js';}</script>
